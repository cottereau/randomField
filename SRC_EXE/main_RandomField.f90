...
!    double precision  , dimension(:) , allocatable :: VecB, VecC
!    integer :: sizeM = 3, sizeN = 4
!
!    allocate(MatA(sizeM, sizeN))
!    allocate(VecB(sizeM), VecC(sizeN))
!
!    MatA = reshape([(1, i=1,sizeM*sizeN)],[sizeM, sizeN])
!    VecB = [(2*i, i=1,sizeM)]
!    VecC = [(2*i-1, i=1,sizeN)]
!
!    call dispCarvalhol(MatA, "MatA");
!    call dispCarvalhol(VecB, "VecB");
!    call dispCarvalhol(VecC, "VecC");
!
!    call DGER  ( sizeM, sizeN, 1.0D0, VecB, 1, VecC, 1, MatA, sizeM )
!
!    call dispCarvalhol(MatA, "MatA after");
!
!    deallocate(MatA)
!    deallocate(VecB, VecC)
!    !END TODO
