...
!
!            !SENDING--------------------------------------------------------------------------------
!            if(.not. snd) call wLog("SENDING--------------------------------------------------------------------------------")
!
!            snd = .true.
!            neighShift = MSH%neighShift(:, neighPos)
!            neighRank  = MSH%neigh(neighPos)
!            !tag = findShiftRang(neighShift)
!            tag = MSH%neigh(neighPos)
!
!            call wLog("neighRank")
!            call wLog(neighRank)
!            call wLog("neighPos")
!            call wLog(neighPos)
!            call wLog("neighShift")
!            call wLog(neighShift)
!            call wLog(" ")
!
!            if(neighRank < 0) snd = .false.
!
!            !if(tag < 0) snd = .false. !Check if this neighbour exists
!            !if(neighpos /= direction) cycle !???
!
!            !Checking if we should send this part of the field to this neighbour
!            if(snd) then
!                do i = 1, MSH%nDim
!                    if(       (neighShift(i) /= 0) &
!                        .and. (neighShift(i) /= dirShift(i))) then
!                        snd = .false.
!                        exit
!                    end if
!                end do
!            end if
!
!            if(.not. snd) call wLog("NOT SENDING")
!
!            !if(MSH%neigh(neighPos) == 0) cycle !FOR TESTS
!
!            if(snd) then
...
!
!            !RECEIVING--------------------------------------------------------------------------------
!            if(.not. snd) call wLog("RECEIVING--------------------------------------------------------------------------------")
!            rcv = .true.
!            op_neighShift = -neighShift
!            op_dirShift   = -dirShift
!            op_neighRank  = findShiftRang(MSH, op_neighShift)
!            !tag = findShiftRang(op_neighShift)
!            tag = myRank
!
!            call wLog("op_neighRank")
!            call wLog(op_neighRank)
!            call wLog("op_neighShift")
!            call wLog(op_neighShift)
!            call wLog("op_dirShift")
!            call wLog(op_dirShift)
!            call wLog(" ")
!
!            if(op_neighRank < 0) rcv = .false.
!
!            !if(tag < 0) rcv = .false. !Check if this neighbour exists
!
!            if(rcv) then
!                do i = 1, MSH%nDim
!                    if(       (op_neighShift(i) /= 0) &
!                        .and. (op_neighShift(i) /= op_dirShift(i))) then
!                        rcv = .false.
!                        exit
!                    end if
!                end do
!            end if
!
!            if(.not. rcv) call wLog("NOT RECEIVING")
!
!            !op_direction = direction + 1
!            !op_neighPos  = neighPos + 1
!            !if(mod(direction,2) == 0) op_direction = direction - 1
!            !if(mod(neighPos,2) == 0)  op_neighPos  = neighPos - 1
!
!
!            if(rcv) then
!
!                !!RECEIVING FROM OPOSITE DIRECTION
!
!                !if(neighPos /= testrank) cycle !FOR TESTS
!                !if(neighPos /= testrank2) cycle !FOR TESTS
!                !if(neighPos /= testrank .and. neighPos /= testrank2) cycle !FOR TESTS
...
!
!                !tag = myRank
!                !tag = findTag(MSH, op_neighPos, op_direction, send = .false.)
!                call wLog(" --------------------")
!                call wLog("Tag rcv in dir ")
...
!                    call MPI_RECV (TRF_2D(minPos(1):maxPos(1),minPos(2):maxPos(2)), &
!                        totalSize, MPI_DOUBLE_PRECISION, &
!                        MSH%neigh(op_neighPos), tag, RDF%comm, status, code)
!                    !RDF%randField(minPos:maxPos,1) = RDF%randField(minPos:maxPos,1) + tempRandField(minPos:maxPos,1)
!                    !call dispCarvalhol(TRF_2D(minPos(1):maxPos(1),minPos(2):maxPos(2)), &
!                    !                   "TRF_2D(minPos(1):maxPos(1),minPos(2):maxPos(2))", &
!                    !                   unit_in=RDF%log_ID)
